...
export class StudentService {
  ...
  protected convertDateFromClient(student: IStudent): IStudent {
    const copy: IStudent = Object.assign({}, student, {
      dateOfBirth: student.dateOfBirth != null && student.dateOfBirth.isValid() ? student.dateOfBirth.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((student: IStudent) => {
        student.dateOfBirth = student.dateOfBirth != null ? moment(student.dateOfBirth) : null;
      });
    }
    return res;
  }
